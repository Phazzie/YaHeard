...
- DONE: Fix Gemini format bug — getSupportedFormats() returns extensions, not MIME types (src/implementations/gemini.ts)
- Normalize confidence handling across processors, API fallback, and UI (see TranscriptionResult.confidence)
- Reduce O(n²) similarity cost in implementations/comparison.ts with a simple pairwise cache
- Add MIME/magic-byte sniffing for uploads (don’t rely on extension only)
...
- Logging: src/lib/logger.ts for structured JSON logs (requestId, route, ip)

## Dev workflow
- Node 20. Scripts: dev, build, preview, check, test
...
## Local gotchas
- Gemini formats mismatch is fixed; keep extensions consistent on any edits
...
- Structured logging exists; propagate logger to new code paths
...
- Tests: npm run test; keep unit tests green and add coverage thresholds in CI
